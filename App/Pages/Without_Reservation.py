...
st.write('---------')
people  = st.number_input('People', 1, 10)
col1 , col2  = st.columns(2)

with col2:
        Category_Radio_Without = st.radio('Choose Category', ["Pizza", "Pasta", "Drink", "Appetizer", "Dessert", "Extra"])

with col1:
    if Category_Radio_Without:
        query_map = {
        ...
        Pizza_query = query_map.get(Category_Radio_Without, "")
...
with col3:
    if st.button("Insert Order Item"):
        try:
            l_status = "Dine in without Reservation"
            l_item_id = item
            l_quantity = quantity
...
with col4:
    ...
    if st.button("Order Completed"):
        try:
            ...
            VALUES (:order_id, TO_DATE(:current_date, 'DD-MM-YY'), TO_TIMESTAMP(:current_time, 'HH:MI:SS PM'), :order_price_result, :people, NULL, 'Dine in without Reservation', :shift, :random_number, NULL ,'Pending')
            """
            
            cursor.execute(order_query, {'order_id': last_order_id, 'current_date': current_date, 'current_time': current2_time, 'order_price_result': order_price_result, 'random_number': random_number , 'shift':shift , 'people':people})
            connection.commit()  
